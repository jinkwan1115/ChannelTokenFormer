...
from exp.exp_long_term_forecasting_practical import Exp_Long_Term_Forecast
...
if __name__ == '__main__':
    ...
    # DUET
    parser.add_argument('--fc_dropout', type=float, default=0.2, help='dropout rate for the final fully connected layer')
    parser.add_argument('--CI', type=int, default=1, help='channel independence')
    parser.add_argument('--k', type=int, default=1, help='number of clusters')
    parser.add_argument('--num_experts', type=int, default=4, help='number of experts in MoE')
    parser.add_argument('--noisy_gating', type=bool, default=True, help='whether to use noisy gating in MoE')
    parser.add_argument('--hidden_size', type=int, default=256, help='hidden size in gating network of MoE')
    ...
    parser.add_argument('--kernel_set', type=list, default=[2,3,6,7])
    parser.add_argument('--bg_dropout', type=float, default=0.3)
    ...
    parser.add_argument('--sampling_periods', type=float, nargs='+', default=[1,1,1,1,1,1,1], help='sampling periods')
    parser.add_argument('--num_global_tokens', type=int, default=1, help='number of global tokens')
    parser.add_argument('--fix_seed', type=int, default=2021, help='gpu')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            setting = '{}_{}_{}_ft{}_sl{}_pl{}_dm{}_nh{}_el{}_dl{}_df{}_bs{}_use_norm{}_ngt{}_des{}_seed{}_lr{}_{}'.format(
                    args.task_name,
                    args.model_id,
                    args.model,
                    args.features,
            ...
            exp.test(setting, missing_ratio=args.missing_ratio)
        ...
        print('>>>>>>>testing : {}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<'.format(setting))
        exp.test(setting, test=1, missing_ratio=args.missing_ratio)
        if args.gpu_type == 'mps':
            torch.backends.mps.empty_cache()
        elif args.gpu_type == 'cuda':
            torch.cuda.empty_cache()
